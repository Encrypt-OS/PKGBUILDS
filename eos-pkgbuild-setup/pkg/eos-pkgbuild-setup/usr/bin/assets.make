...
Assets_clone()
{
    if [ $use_local_assets -eq 1 ] && [ "$REPONAME" != "Encrypt-OS_calamares" ] ; then
        return
    fi
    ...
    case "$REPONAME" in
        Encrypt-OS_calamares) ;;  # many maintainers, so make sure we have the same assets!
        *)
            if [ "$repoup" = "1" ] ; then
...
_pkgbuilds_eos_hook()
{
    # A hook function to make sure local Encrypt-OS PKGBUILDS are up to date.
    ...
    git clone https://github.com/Encrypt-OS/PKGBUILDS.git >& /dev/null || {
        echo2 "Error: $conf: git clone https://github.com/Encrypt-OS/PKGBUILDS.git failed."
        Popd
        exit 1
    }
...
RunPreHooksEOS()
{
    test "$SIGNER" = "Encrypt-OS" && _pkgbuilds_eos_hook
}
...
Main2()
{
    ...
    local use_release_assets         # currently only for [Encrypt-OS] repo
    ...
    if [ -n "$built" ] || [ "$repoup" = "1" ] ; then
        ...
        case "$REPONAME" in
            ...
            *)
                case "$SIGNER" in
                    Encrypt-OS) ManageGithubReleaseAssets ;;
...
ManageGithubNormalFiles() {
    case "$REPONAME" in
        Encrypt-OS) ;;
        Encrypt-OS-testing-dev) return ;;   # TODO: remove 'return' when the repo exists!
        *) return ;;
    esac
...
Main() {
    ...
    if [ -n "$_packager" ] ; then
        ...
        export PACKAGER="Encrypt-OS <info@Encrypt-OS.com>"
    fi
    _packager=""
